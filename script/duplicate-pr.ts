...
import { createCloudpilot } from "@cloudpilot-ai/sdk"
...
`)
  ...
  const cloudpilot = await createCloudpilot({ port: 0 })
  ...
  try {
    ...
    const session = await cloudpilot.client.session.create()
    const result = await cloudpilot.client.session
      .prompt({
