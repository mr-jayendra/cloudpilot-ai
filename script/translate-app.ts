...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "cloudpilot/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".cloudpilot/glossary/zh-cn.md"
  if (locale === "zht") return ".cloudpilot/glossary/zh-tw.md"
  return `.cloudpilot/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("CloudPilot did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("CloudPilot returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("CloudPilot session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel CloudPilot runs for 'all' (default: 4)
      --model <provider/id>  CloudPilot model (default: cloudpilot/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running CloudPilot
...
`)
  ...
  if (failed.length) console.error(`\nCloudPilot failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.CLOUDPILOT_DISABLE_PROJECT_CONFIG = "1"
  env.CLOUDPILOT_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["cloudpilot", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.CLOUDPILOT_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["cloudpilot", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.CLOUDPILOT_CONFIG
  delete env.CLOUDPILOT_CONFIG_DIR
  delete env.CLOUDPILOT_CONFIG_CONTENT
  delete env.CLOUDPILOT_PERMISSION
  delete env.CLOUDPILOT_AUTO_SHARE
  return env
}
