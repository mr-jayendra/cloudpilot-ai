...
This is the living design and status document for `@cloudpilot-ai/codemode` and its existing V2 CloudPilot adapter.
...
### V2 CloudPilot adapter

This section describes the `v2` branch integration. On `dev`, CodeMode is integrated through
`packages/cloudpilot-ai/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
