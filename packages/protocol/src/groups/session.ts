...
import { PromptInput } from "@cloudpilot-ai/schema/prompt-input"
import { Session } from "@cloudpilot-ai/schema/session"
import { Project } from "@cloudpilot-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@cloudpilot-ai/schema/schema"
...
import { Revert } from "@cloudpilot-ai/schema/revert"
import { SessionEvent } from "@cloudpilot-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this CloudPilot process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this CloudPilot process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
