...
Practical reference for the current tool-migration state in `packages/cloudpilot-ai`.
...
Tool tests should use the existing Effect helpers in `packages/cloudpilot-ai/test/lib/effect.ts`:
