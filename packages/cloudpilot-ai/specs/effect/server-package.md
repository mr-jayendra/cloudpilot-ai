...
Practical reference for a future `packages/server` split after the cloudpilot
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/cloudpilot-ai`.
...
Until enough shared service code lives outside `packages/cloudpilot-ai`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/cloudpilot-ai`

It should not import `packages/cloudpilot-ai` services while `packages/cloudpilot-ai`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on cloudpilot-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/cloudpilot-ai` implementation details.
