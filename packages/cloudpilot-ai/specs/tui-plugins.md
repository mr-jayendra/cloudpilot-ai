...
- Author package entrypoint is `@cloudpilot-ai/plugin/tui`.
...
```json
{
  ...
  "plugin": ["@acme/cloudpilot-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@cloudpilot-ai/plugin/tui`.
- `@cloudpilot-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@cloudpilot-ai/plugin/tui"
...
- The value is a semver range checked against the running CloudPilot version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.cloudpilot` is absent, no check is performed (backward compatible).
...
- `cloudpilot plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `cloudpilot plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.cloudpilot` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.cloudpilot`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.cloudpilot`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.cloudpilot` dir or `CLOUDPILOT_CONFIG_DIR`, CloudPilot installs `@cloudpilot-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@cloudpilot-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus CloudPilot's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
CloudPilot registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, CloudPilot pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@cloudpilot-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"cloudpilot"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `cloudpilot.default`.
...
- Local plugins persist installed themes under the local `.cloudpilot/themes` area near the plugin config source.
...
- `--pure` / `CLOUDPILOT_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.cloudpilot/plugins/tui-smoke.tsx`
- Local vim plugin: `.cloudpilot/plugins/tui-vim.tsx`
- Local smoke config: `.cloudpilot/tui.json`
- Local smoke theme: `.cloudpilot/plugins/smoke-theme.json`
