...
Delete `packages/cloudpilot-ai/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/cloudpilot-ai/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/cloudpilot-ai/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/cloudpilot-ai`
- TUI plugin loader tests in `packages/cloudpilot-ai` if runtime plugin API wiring changed
