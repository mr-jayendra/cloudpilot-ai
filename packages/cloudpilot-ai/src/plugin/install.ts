...
export type PatchDeps = {
  ...
  files: (dir: string, name: "cloudpilot" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".cloudpilot")
}

function patchName(kind: Kind): "cloudpilot" | "tui" {
  if (kind === "server") return "cloudpilot"
  return "tui"
}
