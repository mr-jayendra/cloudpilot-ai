...
export const DEPRECATED_PLUGIN_PACKAGES = ["cloudpilot-openai-codex-auth", "cloudpilot-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, cloudpilotVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(cloudpilotVersion) || semver.major(cloudpilotVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.cloudpilot
  if (typeof range !== "string") return
  if (!semver.satisfies(cloudpilotVersion, range)) {
    throw new Error(`Plugin requires cloudpilot ${range} but running ${cloudpilotVersion}`)
  }
}
