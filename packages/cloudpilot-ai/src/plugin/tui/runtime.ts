...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@cloudpilot-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@cloudpilot-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@cloudpilot-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@cloudpilot-ai/tui/context/theme"
import { Global } from "@cloudpilot-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@cloudpilot-ai/core/util/flock"
import { Flag } from "@cloudpilot-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@cloudpilot-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@cloudpilot-ai/core/v1/config/plugin"
import { createCommandShim } from "@cloudpilot-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@cloudpilot-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".cloudpilot"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".cloudpilot", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".cloudpilot", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.CLOUDPILOT_PURE ? [] : pluginOrigins
    if (Flag.CLOUDPILOT_PURE && pluginOrigins.length) {
    }
