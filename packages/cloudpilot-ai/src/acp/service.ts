...
import type { AssistantMessage, Message, CloudpilotClient, SessionMessageResponse } from "@cloudpilot-ai/sdk/v2"
...
export const AuthMethodID = "cloudpilot-login"
...
export class Service extends Context.Service<Service, Interface>()("@cloudpilot/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `cloudpilot auth login` in the terminal",
      name: "Login with cloudpilot",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: CloudpilotClient) {
...
function makeUsageService(sdk: CloudpilotClient) {
...
async function loadDirectorySnapshot(sdk: CloudpilotClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, cloudpilot provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const cloudpilotProvider = providers[ProviderV2.ID.make("cloudpilot")]
  const cloudpilotModel = cloudpilotProvider ? Provider.sort(Object.values(cloudpilotProvider.models))[0] : undefined
  if (cloudpilotProvider && cloudpilotModel) return { providerID: cloudpilotProvider.id, modelID: cloudpilotModel.id }
...
function promptErrorMessage(error: AssistantError) {
  ...
  return "CloudPilot prompt failed"
}
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "CloudPilot service failure", service })
}
