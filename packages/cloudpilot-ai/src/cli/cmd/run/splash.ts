...
//
// Renders the full cloudpilot entry logo and a compact [O] exit badge, plus
...
export const CLOUD_PILOT_TAGLINE = "multi-agent cloud operational platform"
...
function build(input: SplashWriterInput, kind: "entry" | "exit", ctx: ScrollbackRenderContext): ScrollbackSnapshot {
  ...
  if (kind === "entry") {
    const top = 1
    const body_left = 2
    const tagline = input.theme.tagline ?? left

    push(lines, body_left, top, "CloudPilot", right, undefined, TextAttributes.BOLD)
    push(
      lines,
      body_left,
      top + 1,
      Locale.truncateMiddle(CLOUD_PILOT_TAGLINE, Math.max(1, width - body_left)),
      tagline,
      undefined,
    )
    if (input.detail) {
  ...
  if (kind === "exit") {
    ...
    push(
      ...
      top + 1,
      `cloudpilot --mini -s ${meta.session_id}`,
      right,
      undefined,
      TextAttributes.BOLD,
    )
