...
import type { TuiThemeCurrent } from "@cloudpilot-ai/plugin/tui"
...
export function resolveTheme(theme: ThemeJson, pick: "dark" | "light"): TuiThemeCurrent {
  ...
  return {
    ...
    backgroundSelected:
      theme.theme.backgroundSelected === undefined ? resolved.primary! : resolveColor(theme.theme.backgroundSelected),
...
function splashTheme(theme: TuiThemeCurrent, indexed: RGBA[]): RunSplashTheme {
  ...
  const tagline = nearestIndexed(indexed, theme.success)
  return {
    left,
    right,
...
export async function resolveRunTheme(renderer: CliRenderer): Promise<RunTheme> {
  try {
    ...
    const shared = await import("@cloudpilot-ai/tui/context/theme")
