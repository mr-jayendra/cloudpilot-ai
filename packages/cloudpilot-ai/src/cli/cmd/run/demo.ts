...
import type { Event, ToolPart } from "@cloudpilot-ai/sdk/v2"
...
function emitTask(state: State): void {
  ...
  const part = {
    ...
    state: {
      status: "running",
      input: {
        filePath: "packages/cloudpilot-ai/src/cli/cmd/run/stream.ts",
        offset: 1,
        limit: 200,
      },
      time: {
  ...
  if (kind === "read") {
    ...
    askPermission(state, {
      ...
      always: [target],
        ...
        title: "read",
        output: ["1: {", '2:   "name": "cloudpilot",', '3:   "private": true', "4: }"].join("\n"),
        metadata: {},
      },
    })
    return
