// Shared type vocabulary for the direct interactive mode (`cloudpilot --mini`).
//
...
import type { CloudpilotClient, PermissionRequest, QuestionRequest, ToolPart } from "@cloudpilot-ai/sdk/v2"
import type { TuiConfig } from "@cloudpilot-ai/tui/config"
...
type PromptModel = Parameters<CloudpilotClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<CloudpilotClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<CloudpilotClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<CloudpilotClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<CloudpilotClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<CloudpilotClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<CloudpilotClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<CloudpilotClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<CloudpilotClient["question"]["reject"]>[0]
