import type { PermissionV1 } from "@cloudpilot-ai/core/v1/permission"
import { FSUtil } from "@cloudpilot-ai/core/fs-util"
// CLI entry point for `cloudpilot run` and `cloudpilot --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`cloudpilot --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`cloudpilot --mini --attach`): connects to a running
//      cloudpilot server and runs interactive mode against it.
//
...
import { createCloudpilotClient, type CloudpilotClient, type ToolPart } from "@cloudpilot-ai/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<CloudpilotClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run cloudpilot with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running cloudpilot server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to CLOUDPILOT_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to CLOUDPILOT_SERVER_USERNAME or 'cloudpilot')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: CloudpilotClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: CloudpilotClient, sessionID: string) {
      ...
      async function current(sdk: CloudpilotClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: CloudpilotClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: CloudpilotClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: CloudpilotClient) {
        ...
        async function loop(client: CloudpilotClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createCloudpilotClient({
        baseUrl: "http://cloudpilot.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
