...
import type { Hooks } from "@cloudpilot-ai/plugin"
...
export const ProvidersLoginCommand = effectCmd({
  ...
  builder: (yargs: Argv) =>
    yargs
      .positional("url", {
        describe: "cloudpilot auth provider",
        type: "string",
      })
      .option("provider", {
        alias: ["p"],
  ...
  handler: Effect.fn("Cli.providers.login")(function* (args) {
    ...
    if (args.url) {
      ...
      const wellknown = (yield* cliTry(`Failed to load auth provider metadata from ${url}: `, () =>
        fetch(`${url}/.well-known/cloudpilot`).then((x) => x.json()),
      )) as {
    ...
    if (provider === "other") {
      ...
      yield* Prompt.log.warn(
        `This only stores a credential for ${provider} - you will need configure it in cloudpilot.json, check the docs for examples.`,
      )
    }
    ...
    if (provider === "amazon-bedrock") {
      yield* Prompt.log.info(
        "Amazon Bedrock authentication priority:\n" +
          ...
          "Configure via cloudpilot.json options (profile, region, endpoint) or\n" +
          "AWS environment variables (AWS_PROFILE, AWS_REGION, AWS_ACCESS_KEY_ID, AWS_WEB_IDENTITY_TOKEN_FILE).",
      )
    }

    if (provider === "cloudpilot") {
      yield* Prompt.log.info("Create an api key from your account console")
    }
