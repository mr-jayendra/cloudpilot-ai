...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall cloudpilot and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall CloudPilot")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g cloudpilot-ai",
      pnpm: "pnpm uninstall -g cloudpilot-ai",
      bun: "bun remove -g cloudpilot-ai",
      yarn: "yarn global remove cloudpilot-ai",
      brew: "brew uninstall cloudpilot",
      choco: "choco uninstall cloudpilot",
      scoop: "scoop uninstall cloudpilot",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "cloudpilot", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using CloudPilot!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# cloudpilot") || content.includes(".cloudpilot/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".cloudpilot/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".cloudpilot/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".cloudpilot"))
    ) {
      continue
    }
