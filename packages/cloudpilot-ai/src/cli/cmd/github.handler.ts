...
const AGENT_USERNAME = "cloudpilot-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/cloudpilot.yml"
      ...
      function printNextSteps() {
        ...
        prompts.outro(
          [
            ...
            "",
            "   Learn more about the GitHub agent in the provider documentation.",
          ].join("\n"),
        )
      }
...
jobs:
  cloudpilot:
    if: |
      ...
      contains(github.event.comment.body, ' /cloudpilot') ||
      startsWith(github.event.comment.body, '/cloudpilot')
...
export const githubRun = Effect.fn("Cli.github.run")(function* (args: { event?: string; token?: string }) {
  ...
  yield* Effect.promise(async () => {
    ...
    try {
      ...
      console.log("cloudpilot session", session.id)
    ...
    async function getUserPrompt() {
      ...
      const mentions = (process.env["MENTIONS"] || "/cloudpilot,/oc")
        .split(",")
    ...
    async function chat(message: string, files: PromptFiles = []) {
      console.log("Sending message to cloudpilot...")
    ...
    async function getOidcToken() {
      try {
        return await core.getIDToken("cloudpilot-github-action")
      } catch (error) {
    ...
    function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
      ...
      if (type === "schedule" || type === "dispatch") {
        ...
        return `cloudpilot/${type}-${hex}-${timestamp}`
      }
      return `cloudpilot/${type}${issueId}-${timestamp}`
    }
    ...
    function footer() {
      const shareUrl = shareId ? `[cloudpilot session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
  ...
  repository(owner: $owner, name: $repo) {
    ...
    function buildPromptDataForIssue(issue: GitHubIssue) {
      ...
      return [
        ...
        "- Git push and PR creation are handled AUTOMATICALLY by the cloudpilot infrastructure after your response",
