...
import { FrontmatterError } from "@cloudpilot-ai/core/v1/config/error"
import { ConfigMarkdown } from "@/config/markdown"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Glob } from "@cloudpilot-ai/core/util/glob"
...
const CLOUDPILOT_SKILL_PATTERN = "{skill,skills}/**/SKILL.md"
const SKILL_PATTERN = "**/SKILL.md"

// Built-in skill that ships with cloudpilot. The model's intuition for what an
// cloudpilot.json should look like is often wrong, and cloudpilot hard-fails on
// invalid config, so users hit cryptic startup errors. Loading this skill
// when the model is asked to touch cloudpilot's own config files gives it the
// actual schemas instead of guesses.
const CUSTOMIZE_CLOUDPILOT_SKILL_NAME = "customize-cloudpilot"
const CUSTOMIZE_CLOUDPILOT_SKILL_DESCRIPTION =
  "Use ONLY when the user is editing or creating cloudpilot's own configuration: cloudpilot.json, cloudpilot.jsonc, files under .cloudpilot/, or files under ~/.config/cloudpilot/. Also use when creating or fixing cloudpilot agents, subagents, skills, plugins, MCP servers, or permission rules. Do not use for the user's own application code, or for any project that is not configuring cloudpilot itself."
const CUSTOMIZE_CLOUDPILOT_SKILL_BODY = SkillPlugin.CustomizeCloudpilotContent
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, CLOUDPILOT_SKILL_PATTERN)
  }
...
export class Service extends Context.Service<Service, Interface>()("@cloudpilot/Skill") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("Skill.state")(function* () {
        ...
        s.skills[CUSTOMIZE_CLOUDPILOT_SKILL_NAME] = {
          name: CUSTOMIZE_CLOUDPILOT_SKILL_NAME,
          description: CUSTOMIZE_CLOUDPILOT_SKILL_DESCRIPTION,
          location: "<built-in>",
          content: CUSTOMIZE_CLOUDPILOT_SKILL_BODY,
        }
