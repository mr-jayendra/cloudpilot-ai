...
import type { LLMClientShape } from "@cloudpilot-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("cloudpilot"))
    return { type: "unsupported", reason: "provider is not openai, cloudpilot, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @cloudpilot-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains cloudpilot-owned. The native runtime only adapts
      // the @cloudpilot-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
