...
import type * as SDK from "@cloudpilot-ai/sdk/v2"
...
import { SessionShareTable } from "@cloudpilot-ai/core/share/sql"
...
const disabled = process.env["CLOUDPILOT_DISABLE_SHARE"] === "true" || process.env["CLOUDPILOT_DISABLE_SHARE"] === "1"
...
export class Service extends Context.Service<Service, Interface>()("@cloudpilot/ShareNext") {}
  ...
  Effect.gen(function* () {
    ...
    const request = Effect.fn("ShareNext.request")(function* () {
      ...
      if (Option.isNone(active) || !active.value.active_org_id) {
        const baseUrl = (yield* cfg.get()).enterprise?.url ?? "https://mr-jayendra.github.io/cloudpilot-ai"
