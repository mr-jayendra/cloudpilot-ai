...
import { CloudPilotHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for cloudpilot serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(CloudPilotHttpApi)
...
export const PublicApi = CloudPilotHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "cloudpilot",
    version: "1.0.0",
    description: "cloudpilot api",
