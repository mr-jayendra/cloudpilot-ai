...
import { isRecord } from "@cloudpilot-ai/tui/util/record"
...
import { TuiKeybind } from "@cloudpilot-ai/tui/config/keybind"
...
import { TuiConfig } from "@cloudpilot-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@cloudpilot/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old cloudpilot.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.cloudpilot`
  // folders between cwd and home, and CLOUDPILOT_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.CLOUDPILOT_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit CLOUDPILOT_TUI_CONFIG override, if set.
  if (Flag.CLOUDPILOT_TUI_CONFIG) {
    const configFile = Flag.CLOUDPILOT_TUI_CONFIG
  ...
  // 4. `.cloudpilot` directories (and CLOUDPILOT_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".cloudpilot") || dir === Flag.CLOUDPILOT_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".cloudpilot") && dir !== Flag.CLOUDPILOT_CONFIG_DIR) continue
