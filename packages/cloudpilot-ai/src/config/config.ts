...
import { serviceUse } from "@cloudpilot-ai/core/effect/service-use"
...
import { InstallationLocal, InstallationVersion } from "@cloudpilot-ai/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@cloudpilot-ai/core/v1/config/console-state"
import { FSUtil } from "@cloudpilot-ai/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@cloudpilot-ai/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@cloudpilot-ai/core/v1/config/config"
import { RemoteAuthError } from "@cloudpilot-ai/core/v1/config/error"
import { ConfigPermissionV1 } from "@cloudpilot-ai/core/v1/config/permission"
...
export class Service extends Context.Service<Service, Interface>()("@cloudpilot/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["cloudpilot.jsonc", "cloudpilot.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://mr-jayendra.github.io/cloudpilot-ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.CLOUDPILOT_CONFIG && !Flag.CLOUDPILOT_CONFIG_DIR && !Flag.CLOUDPILOT_CONFIG_CONTENT) {
        const file = globalConfigFile()
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://mr-jayendra.github.io/cloudpilot-ai/config.json" }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "cloudpilot.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "cloudpilot.jsonc"), env))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://mr-jayendra.github.io/cloudpilot-ai/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "CLOUDPILOT_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/cloudpilot`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://mr-jayendra.github.io/cloudpilot-ai/config.json"
        ...
        if (Flag.CLOUDPILOT_CONFIG) {
          yield* merge(Flag.CLOUDPILOT_CONFIG, yield* loadFile(Flag.CLOUDPILOT_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.CLOUDPILOT_CONFIG })
        }

        if (!Flag.CLOUDPILOT_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("cloudpilot", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.CLOUDPILOT_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from CLOUDPILOT_CONFIG_DIR", { path: Flag.CLOUDPILOT_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".cloudpilot") || dir === Flag.CLOUDPILOT_CONFIG_DIR) {
            for (const file of ["cloudpilot.json", "cloudpilot.jsonc"]) {
          ...
          // Auto-discovered plugins under `.cloudpilot/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.CLOUDPILOT_CONFIG_CONTENT) {
          const source = "CLOUDPILOT_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.CLOUDPILOT_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from CLOUDPILOT_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["CLOUDPILOT_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("CLOUDPILOT_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.CLOUDPILOT_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.CLOUDPILOT_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("CLOUDPILOT_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.CLOUDPILOT_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.CLOUDPILOT_DISABLE_PRUNE) {
