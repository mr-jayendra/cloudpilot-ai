...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from cloudpilot.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const cloudpilot = await cloudpilotFiles(input)
  for (const file of cloudpilot) {
...
async function cloudpilotFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "cloudpilot"),
    ...(await Filesystem.findUp(["cloudpilot.json", "cloudpilot.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "cloudpilot"))
  }
  if (Flag.CLOUDPILOT_CONFIG) files.push(Flag.CLOUDPILOT_CONFIG)
