...
import { NonNegativeInt, PositiveInt } from "@cloudpilot-ai/core/schema"
import { ConfigAttachmentV1 } from "@cloudpilot-ai/core/v1/config/attachment"
import { ConfigLSPV1 } from "@cloudpilot-ai/core/v1/config/lsp"
import { InvalidError } from "@cloudpilot-ai/core/v1/config/error"
  ...
  if (permissions.length)
    throw new InvalidError({
      ...
      issues: permissions.map((path) => ({
        path,
        message: 'V2 permissions are not supported by CloudPilot V1. Use V1 "permission" rules or run cloudpilot2.',
      })),
    })
