    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".cloudpilot/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const cloudpilotModel = {
      ...openaiModel,
      providerID: "cloudpilot",
      api: {
        id: "cloudpilot-test",
    ...
    const result = ProviderTransform.message(msgs, cloudpilotModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.cloudpilot?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.cloudpilot?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          cloudpilot: { itemId: "msg_cloudpilot" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              cloudpilot: { itemId: "msg_cloudpilot_part" },
    ...
    expect(result[0].providerOptions?.cloudpilot?.itemId).toBe("msg_cloudpilot")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.cloudpilot?.itemId).toBe("msg_cloudpilot_part")
...
describe("ProviderTransform sampling defaults - DeepSeek", () => {
  ...
  test.each([
    ...
    ["cloudpilot", "deepseek-v4-flash"],
    ["cloudpilot-go", "deepseek-v4-flash"],
