  ...
  Effect.gen(function* () {
    ...
    yield* Effect.addFinalizer(() =>
      Effect.sync(() => {
        ...
        restoreEnv("CLOUDPILOT_SERVER_PASSWORD", original.envPassword)
        restoreEnv("CLOUDPILOT_SERVER_USERNAME", original.envUsername)
      }),
    )
  }),
)
...
function authConfigLayer(input?: { password?: string; username?: string }) {
  return ServerAuth.Config.configLayer({
    ...
    username: input?.username ?? "cloudpilot",
  })
}
      ...
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({
            CLOUDPILOT_SERVER_PASSWORD: input?.password,
            CLOUDPILOT_SERVER_USERNAME: input?.username,
          }),
        ),
      ),
    ),
...
describe("HttpApi UI fallback", () => {
  ...
  it.live("allows web UI preflight without auth", () =>
    Effect.gen(function* () {
      const response = yield* app({ password: "secret", username: "cloudpilot" }).request("/", {
        method: "OPTIONS",
        headers: {
