import { PermissionV1 } from "@cloudpilot-ai/core/v1/permission"
...
import { SessionInputTable, SessionMessageTable, SessionTable } from "@cloudpilot-ai/core/session/sql"
...
const originalWorkspaces = Flag.CLOUDPILOT_EXPERIMENTAL_WORKSPACES
...
afterEach(async () => {
  Flag.CLOUDPILOT_EXPERIMENTAL_WORKSPACES = originalWorkspaces
...
describe("session HttpApi", () => {
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const headers = { "x-cloudpilot-directory": test.directory }
    ...
    () =>
      Effect.gen(function* () {
        ...
        const messages = yield* request(`/api/session/${session.id}/message`, {
          headers: { "x-cloudpilot-directory": test.directory },
        })
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const headers = { "x-cloudpilot-directory": test.directory, "content-type": "application/json" }
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        Flag.CLOUDPILOT_EXPERIMENTAL_WORKSPACES = true
        ...
        const created = yield* requestJson<Session.Info>(`${SessionPaths.create}?workspace=${workspace.id}`, {
          method: "POST",
          headers: { "x-cloudpilot-directory": test.directory, "content-type": "application/json" },
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const currentDir = path.join(test.directory, "packages", "cloudpilot", "src")
        ...
        const query = new URLSearchParams({
          scope: "project",
          path: "packages/cloudpilot-ai/src",
          directory: currentDir,
        })
        const headers = { "x-cloudpilot-directory": test.directory }
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const headers = { "x-cloudpilot-directory": hint, "content-type": "application/json" }
        const created = yield* requestJson<Session.Info>(SessionPaths.create, {
          method: "POST",
          headers,
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const headers = { "x-cloudpilot-directory": globalWorktreeSentinel, "content-type": "application/json" }
