...
import { createCloudpilotClient } from "@cloudpilot-ai/sdk/v2"
...
type Sdk = ReturnType<typeof createCloudpilotClient>
...
function serverFetch(
  ...
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.CLOUDPILOT_SERVER_PASSWORD = input?.password
      Flag.CLOUDPILOT_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return FSUtil.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".cloudpilot", "skills", "project-rest-skill", "SKILL.md"),
      `---
  ...
  httpapi(
    ...
    withProject("raw", { setup: writeStandardFiles }, ({ directory }) =>
      Effect.gen(function* () {
        ...
        expect(request!.headers.has("x-cloudpilot-directory")).toBe(false)
        expect(request!.headers.has("x-cloudpilot-workspace")).toBe(false)
      }),
    ),
  )
    ...
    ({ directory }) =>
      Effect.gen(function* () {
        ...
        const badSdk = yield* client("raw", directory, {
          password: "secret",
          headers: { authorization: authorization("cloudpilot", "wrong") },
        })
        const bad = yield* capture(() => badSdk.file.read({ path: "hello.txt" }))
        const goodSdk = yield* client("raw", directory, {
          password: "secret",
          headers: { authorization: authorization("cloudpilot", "secret") },
        })
