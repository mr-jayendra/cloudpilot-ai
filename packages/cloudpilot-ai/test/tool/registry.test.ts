...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".cloudpilot")])),
})
...
describe("tool.registry", () => {
  ...
  it.instance("loads tools from .cloudpilot/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const cloudpilot = path.join(test.directory, ".cloudpilot")
      const tool = path.join(cloudpilot, "tool")
  ...
  it.instance("ignores non-tool exports in .cloudpilot/tool files", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".cloudpilot", "tool")
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  // came in through `plugin.list()` — `oh-my-cloudpilot` was registering a tool
  ...
  it.instance("loads tools from .cloudpilot/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const cloudpilot = path.join(test.directory, ".cloudpilot")
      const tools = path.join(cloudpilot, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".cloudpilot", "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const cloudpilot = path.join(test.directory, ".cloudpilot")
        const customTools = path.join(cloudpilot, "tools")
        const plugin = path.join(cloudpilot, "node_modules", "@cloudpilot-ai", "plugin")
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(path.dirname(fileURLToPath(import.meta.resolve("zod"))), path.join(cloudpilot, "node_modules", "zod"), {
            dereference: true,
            recursive: true,
          }),
        )
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@cloudpilot-ai/plugin", type: "module", exports: { ".": "./dist/index.js" } }),
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".cloudpilot", "tools")
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(cloudpilot, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
              "@cloudpilot-ai/plugin": "^0.0.0",
              cowsay: "^1.6.0",
            },
          }),
        ),
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(cloudpilot, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(cloudpilot, "node_modules", "cowsay")
