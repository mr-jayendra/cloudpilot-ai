...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.CLOUDPILOT_WEBSEARCH_PROVIDER

    try {
      process.env.CLOUDPILOT_WEBSEARCH_PROVIDER = "parallel"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("parallel")

      process.env.CLOUDPILOT_WEBSEARCH_PROVIDER = "exa"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("exa")
    } finally {
      if (original === undefined) delete process.env.CLOUDPILOT_WEBSEARCH_PROVIDER
      else process.env.CLOUDPILOT_WEBSEARCH_PROVIDER = original
    }
  })
  ...
  test("is enabled for CloudPilot providers or explicit websearch provider flags", () => {
    expect(webSearchEnabled(ProviderV2.ID.cloudpilot, { exa: false, parallel: false })).toBe(true)
    expect(webSearchEnabled(ProviderV2.ID.make("cloudpilot-go"), { exa: false, parallel: false })).toBe(true)
