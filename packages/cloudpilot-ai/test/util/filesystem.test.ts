...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "cloudpilot.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "cloudpilot.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "cloudpilot.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "cloudpilot.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["cloudpilot.json", "cloudpilot.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "cloudpilot.json"),
        path.join(project, "cloudpilot.jsonc"),
      ])
    })
  })
