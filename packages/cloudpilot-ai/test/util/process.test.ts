...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.CLOUDPILOT_TEST ?? "")'), {
      env: {
        CLOUDPILOT_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "CLOUDPILOT_TEST_SHELL"], {
      shell: true,
      env: {
        CLOUDPILOT_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("CLOUDPILOT_TEST_SHELL=ok")
  })
