...
import { ConfigV1 } from "@cloudpilot-ai/core/v1/config/config"
...
import { NamedError } from "@cloudpilot-ai/core/util/error"
...
import { CrossSpawnSpawner } from "@cloudpilot-ai/core/cross-spawn-spawner"
...
import { ProjectV2 } from "@cloudpilot-ai/core/project"
import { Filesystem } from "@/util/filesystem"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@cloudpilot-ai/core/v1/config/plugin"
...
function remoteConfigClient(input: {
  ...
  return HttpClient.make((request) => {
    if (request.url.includes(".well-known/cloudpilot")) {
...
const schemaConfig = (config: object) => ({ $schema: "https://mr-jayendra.github.io/cloudpilot-ai/config.json", ...config })
...
const managedConfigDir = process.env.CLOUDPILOT_TEST_MANAGED_CONFIG_DIR!
const originalTestToken = process.env.TEST_TOKEN
const originalConsoleToken = process.env.CLOUDPILOT_CONSOLE_TOKEN
...
afterEach(async () => {
  ...
  if (originalConsoleToken === undefined) delete process.env.CLOUDPILOT_CONSOLE_TOKEN
  else process.env.CLOUDPILOT_CONSOLE_TOKEN = originalConsoleToken
  await clear(true)
})

const writeManagedSettingsEffect = (settings: object, filename?: string) =>
  FSUtil.use.writeWithDirs(path.join(managedConfigDir, filename ?? "cloudpilot.json"), JSON.stringify(settings))

async function writeConfig(dir: string, config: object, name = "cloudpilot.json") {
  await Filesystem.write(path.join(dir, name), JSON.stringify(config))
}

const writeConfigEffect = (dir: string, config: object, name = "cloudpilot.json") =>
  ...
  Effect.gen(function* () {
    ...
    yield* Effect.all(
      [
        ...
        input.local ? writeConfigEffect(path.join(directory, ".cloudpilot"), schemaConfig(input.local)) : undefined,
...
it.effect("creates global jsonc config with schema when no global configs exist", () =>
  withGlobalConfig({}, ({ dir }) =>
    Effect.gen(function* () {
      ...
      const content = yield* FSUtil.use.readFileString(path.join(dir, "cloudpilot.jsonc"))
      expect(content).toContain('"$schema": "https://mr-jayendra.github.io/cloudpilot-ai/config.json"')
    }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
  ),
)

it.effect("does not create global config when CLOUDPILOT_CONFIG_DIR is set", () =>
  Effect.gen(function* () {
    const custom = yield* tmpdirScoped()
    yield* withGlobalConfig({}, ({ dir }) =>
      withProcessEnv(
        "CLOUDPILOT_CONFIG_DIR",
        custom,
        Effect.gen(function* () {
          yield* Config.use.get().pipe(provideInstanceEffect(dir))

          expect(yield* FSUtil.use.existsSafe(path.join(dir, "cloudpilot.jsonc"))).toBe(false)
        }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
      ),
    )
  }),
...
it.instance("updates config and preserves empty shell sentinel", () =>
  Effect.gen(function* () {
    ...
    yield* writeConfigEffect(
      test.directory,
      { $schema: "https://mr-jayendra.github.io/cloudpilot-ai/config.json", shell: "bash" },
      "config.json",
    )
...
it.effect("updates global config and omits empty shell key in json", () =>
  withGlobalConfig({ config: { shell: "bash" } }, ({ dir }) =>
    Effect.gen(function* () {
      ...
      const writtenConfig = yield* FSUtil.use.readJson(path.join(dir, "cloudpilot.json"))
...
it.effect("updates global config and omits empty shell key in jsonc", () =>
  withGlobalConfig({ config: { shell: "bash", model: "test/model" }, name: "cloudpilot.jsonc" }, ({ dir }) =>
    Effect.gen(function* () {
      yield* Config.use.updateGlobal({ shell: "" })

      const file = path.join(dir, "cloudpilot.jsonc")
        ...
        expect(
          ...
          [
            ...
            expect.objectContaining({
              source: path.join(dir, "cloudpilot.json"),
              kind: "unsupported",
              path: ["providers"],
            }),
          ],
  ...
  it.live(`fixture ${name}`, () =>
    withGlobalConfig({}, ({ dir }) =>
      Effect.gen(function* () {
        ...
        const file = path.join(dir, `cloudpilot${extension}`)
...
for (const name of ["cloudpilot.json", "cloudpilot.jsonc"]) {
  ...
  withConfigTree(
    ...
    Effect.gen(function* () {
      ...
      if (Exit.isFailure(exit))
        expect(Cause.squash(exit.cause)).toMatchObject({
          data: {
            path: expect.stringContaining(path.join("project", "cloudpilot.json")),
            issues: [
              { path: ["permissions"], message: expect.stringContaining('Use V1 "permission" rules or run cloudpilot2') },
...
it.instance("ignores legacy tui keys in cloudpilot config", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
...
it.instance("loads JSONC config file", () =>
  Effect.gen(function* () {
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, "cloudpilot.jsonc"),
      `{
        // This is a comment
        "$schema": "https://mr-jayendra.github.io/cloudpilot-ai/config.json",
        "model": "test/model",
...
it.instance("preserves env variables when adding $schema to config", () =>
  withProcessEnv(
    ...
    Effect.gen(function* () {
      ...
      yield* FSUtil.use.writeWithDirs(
        path.join(test.directory, "cloudpilot.json"),
      ...
      const content = yield* FSUtil.use.readFileString(path.join(test.directory, "cloudpilot.json"))
...
const accountTokenIt = configIt({
  account: Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { cloudpilot: { options: { apiKey: "{env:CLOUDPILOT_CONSOLE_TOKEN}" } } },
        }),
      ),
...
accountTokenIt.instance("resolves env templates in account config with account token", () =>
  Effect.gen(function* () {
    const config = yield* Config.use.get()
    expect(config.provider?.["cloudpilot"]?.options?.apiKey).toBe("st_test_token")
  }),
)
...
it.instance("throws error for invalid JSON", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(path.join(test.directory, "cloudpilot.json"), "{ invalid json }")
    const exit = yield* Config.use.get().pipe(Effect.exit)
    expect(Exit.isFailure(exit)).toBe(true)
  }),
)
...
it.instance("loads config from .cloudpilot directory", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".cloudpilot", "agent", "test.md"),
      `---
model: test/model
---
Test agent prompt`,
...
it.instance("agent markdown permission config preserves user key order", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".cloudpilot", "agent", "ordered.md"),
      `---
permission:
  bash: allow
  "*": deny
...
it.instance("loads agents from .cloudpilot/agents (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".cloudpilot", "agents", "helper.md"),
      `---
model: test/model
mode: subagent
---
Helper agent prompt`,
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".cloudpilot", "agents", "nested", "child.md"),
      `---
model: test/model
mode: subagent
---
...
it.instance("loads commands from .cloudpilot/command (singular)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".cloudpilot", "command", "hello.md"),
      `---
description: Test command
---
Hello from singular command`,
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".cloudpilot", "command", "nested", "child.md"),
      `---
...
it.instance("loads commands from .cloudpilot/commands (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".cloudpilot", "commands", "hello.md"),
      `---
description: Test command
---
Hello from plural commands`,
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".cloudpilot", "commands", "nested", "child.md"),
      `---
description: Nested command
---
Nested command template`,
...
it.effect("does not try to install dependencies in read-only CLOUDPILOT_CONFIG_DIR", () =>
  Effect.gen(function* () {
    if (process.platform === "win32") return
    ...
    yield* withProcessEnv("CLOUDPILOT_CONFIG_DIR", readonly, Config.use.get().pipe(provideInstanceEffect(dir)))
  }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
)

it.effect("ignores an inaccessible CLOUDPILOT_CONFIG_DIR", () =>
  Effect.gen(function* () {
    if (process.platform === "win32") return
    ...
    yield* withProcessEnv("CLOUDPILOT_CONFIG_DIR", configDir, Config.use.get().pipe(provideInstanceEffect(dir)))
  }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
)

it.effect("creates a missing CLOUDPILOT_CONFIG_DIR", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
...
it.effect("installs dependencies in writable CLOUDPILOT_CONFIG_DIR", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
    const configDir = path.join(dir, "configdir")
    yield* FSUtil.use.ensureDir(configDir)
...
it.effect("global config remains global when project config is disabled", () =>
  withConfigTree(
    ...
    withProcessEnv(
      "CLOUDPILOT_DISABLE_PROJECT_CONFIG",
      "true",
      Effect.gen(function* () {
        const config = yield* Config.use.get()
...
it.instance("does not error when only custom agent is a subagent", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".cloudpilot", "agent", "helper.md"),
      `---
model: test/model
mode: subagent
---
...
// Note: preload.ts sets CLOUDPILOT_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
it.instance("managed jsonc settings override managed json settings", () =>
  Effect.gen(function* () {
    ...
    yield* writeManagedSettingsEffect({ model: "managed/jsonc" }, "cloudpilot.jsonc")
...
it.instance("local .cloudpilot config can override MCP from project config", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* writeConfigEffect(test.directory, {
      $schema: "https://mr-jayendra.github.io/cloudpilot-ai/config.json",
    ...
    yield* FSUtil.use.ensureDir(path.join(test.directory, ".cloudpilot"))
    yield* writeConfigEffect(
      path.join(test.directory, ".cloudpilot"),
      {
        $schema: "https://mr-jayendra.github.io/cloudpilot-ai/config.json",
        mcp: {
          docs: {
      ...
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            ...
            expect(fetchedUrl).toBe(`${server.url.origin}/.well-known/cloudpilot`)
            expect(config.mcp?.jira?.enabled).toBe(true)
          }),
        ),
      { git: true },
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    ...
    const file = path.join(tmp.path, "cloudpilot.json")
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-cloudpilot@2.4.3", file)).toBe("oh-my-cloudpilot@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-cloudpilot@2.4.3", "file:///project/.cloudpilot/plugin/oh-my-cloudpilot.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.cloudpilot/plugin/demo.ts", "file:///project/.cloudpilot/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.cloudpilot/plugin/demo.ts"])
  })
  ...
  it.effect("loads auto-discovered local plugins as file urls", () =>
    withConfigTree(
      ...
      Effect.gen(function* () {
        const test = yield* TestInstance
        yield* FSUtil.use.writeWithDirs(
          path.join(test.directory, ".cloudpilot", "plugin", "my-plugin.js"),
          "export default {}",
        )
...
describe("CLOUDPILOT_DISABLE_PROJECT_CONFIG", () => {
  it.instance(
  ...
  it.instance("skips project .cloudpilot/ directories when flag is set", () =>
    withProcessEnv(
      "CLOUDPILOT_DISABLE_PROJECT_CONFIG",
      "true",
      Effect.gen(function* () {
        const test = yield* TestInstance
        yield* FSUtil.use.writeWithDirs(
          path.join(test.directory, ".cloudpilot", "command", "test-cmd.md"),
  ...
  it.instance(
    ...
    () =>
      withProcessEnvs(
        { CLOUDPILOT_CONFIG_DIR: undefined, CLOUDPILOT_DISABLE_PROJECT_CONFIG: "true" },
        Effect.gen(function* () {
          const test = yield* TestInstance
  ...
  it.instance(
    "CLOUDPILOT_CONFIG_DIR still works when flag is set",
    () =>
      Effect.gen(function* () {
        const configDir = yield* tmpdirScoped({ config: { model: "configdir/model" } })
        yield* withProcessEnvs(
          { CLOUDPILOT_DISABLE_PROJECT_CONFIG: "true", CLOUDPILOT_CONFIG_DIR: configDir },
          Effect.gen(function* () {
            const config = yield* Config.use.get()
...
// Regression for #28206: malformed CLOUDPILOT_PERMISSION JSON used to crash
...
describe("CLOUDPILOT_PERMISSION env var", () => {
  it.instance("does not crash when CLOUDPILOT_PERMISSION contains invalid JSON", () =>
    withProcessEnv(
      "CLOUDPILOT_PERMISSION",
      "{invalid",
      Effect.gen(function* () {
        const config = yield* Config.use.get()
...
describe("CLOUDPILOT_CONFIG_CONTENT token substitution", () => {
  it.instance("substitutes {env:} tokens in CLOUDPILOT_CONFIG_CONTENT", () =>
    withProcessEnv(
      "TEST_CONFIG_VAR",
      "test_api_key_12345",
      withProcessEnv(
        "CLOUDPILOT_CONFIG_CONTENT",
        JSON.stringify({
          $schema: "https://mr-jayendra.github.io/cloudpilot-ai/config.json",
  ...
  it.instance("substitutes {file:} tokens in CLOUDPILOT_CONFIG_CONTENT", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
  ...
  const config = ConfigParse.schema(
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          PayloadDisplayName: "CloudPilot Managed",
          PayloadIdentifier: "ai.cloudpilot.managed.test",
          PayloadType: "ai.cloudpilot.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.schema(
    ConfigV1.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://mr-jayendra.github.io/cloudpilot-ai/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://mr-jayendra.github.io/cloudpilot-ai/config.json")
})
