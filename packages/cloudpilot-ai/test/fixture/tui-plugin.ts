...
import type { HostPluginApi } from "@cloudpilot-ai/tui/plugin/slots"
...
export function createTuiPluginApi(opts: Opts = {}): HostPluginApi {
  ...
  const own = createCloudpilotClient({
  ...
  let selected = opts.theme?.selected ?? "cloudpilot"
  const set =
    opts.theme?.set ??
    ((name: string) => {
  ...
  return {
    ...
    attention: {
      ...
      soundboard: {
        ...
        current: () => opts.attention?.soundboard?.current?.() ?? "cloudpilot.default",
