...
import { EventSequenceTable } from "@cloudpilot-ai/core/event/sql"
...
import { WorkspaceV2 } from "@cloudpilot-ai/core/workspace"
import { WorkspaceTable } from "@cloudpilot-ai/core/control-plane/workspace.sql"
...
import { Ripgrep } from "@cloudpilot-ai/core/ripgrep"
...
const originalEnv = {
  CLOUDPILOT_AUTH_CONTENT: process.env.CLOUDPILOT_AUTH_CONTENT,
  CLOUDPILOT_EXPERIMENTAL_WORKSPACES: process.env.CLOUDPILOT_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.CLOUDPILOT_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@cloudpilot.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.CLOUDPILOT_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=cloudpilot-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.CLOUDPILOT_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.CLOUDPILOT_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.CLOUDPILOT_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=cloudpilot-test")
