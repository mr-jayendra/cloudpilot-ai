...
import type { NamedError } from "@cloudpilot-ai/core/util/error"
...
describe("session.retry.retryable", () => {
  ...
  test("maps free limits to Go upsell action", () => {
    ...
    expect(SessionRetry.retryable(error, "cloudpilot")).toEqual({
      message: SessionRetry.GO_UPSELL_MESSAGE,
      action: {
        reason: "free_tier_limit",
        provider: "cloudpilot",
        title: "Free limit reached",
        message: "Subscribe to CloudPilot Go for reliable access to the best open-source models for $10/month.",
        label: "subscribe",
  ...
  test("maps Go subscription limits without limit metadata", () => {
    ...
    expect(SessionRetry.retryable(error, "cloudpilot-go")?.action?.message).toBe(
