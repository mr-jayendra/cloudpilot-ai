...
import { SessionV1 } from "@cloudpilot-ai/core/v1/session"
import { ModelsDev } from "@cloudpilot-ai/core/models-dev"
import { HttpRecorder } from "@cloudpilot-ai/http-recorder"
import { HttpRecorderInternal } from "@cloudpilot-ai/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@cloudpilot-ai/llm"
import { RequestExecutor } from "@cloudpilot-ai/llm/route"
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.CLOUDPILOT_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["cloudpilot", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("CLOUDPILOT_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("CLOUDPILOT_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["cloudpilot", "native", "oauth", "tool-loop"],
  ...
  {
    id: "cloudpilot-proxy",
    name: "CloudPilot proxy",
    providerID: ProviderV2.ID.cloudpilot,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["cloudpilot", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.CLOUDPILOT_RECORD_CONSOLE_TOKEN && process.env.CLOUDPILOT_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.cloudpilot,
        name: "CloudPilot Zen",
        env: ["CLOUDPILOT_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.CLOUDPILOT_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.CLOUDPILOT_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.CLOUDPILOT_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["cloudpilot", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("CLOUDPILOT_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("CLOUDPILOT_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("CLOUDPILOT_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set CLOUDPILOT_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the cloudpilot LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "cloudpilot.json"),
      JSON.stringify({ $schema: "https://mr-jayendra.github.io/cloudpilot-ai/config.json", ...scenario.config(model) }),
    ),
  )
