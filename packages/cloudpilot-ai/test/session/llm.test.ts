...
import { LLMClient, RequestExecutor } from "@cloudpilot-ai/llm/route"
...
describe("session.llm.stream", () => {
  ...
  const cloudpilotFixture = { providerID: "cloudpilot-test", modelID: vivgridFixture.modelID }

  it.instance(
    "sends the parent session header for cloudpilot providers",
    () =>
      Effect.gen(function* () {
        ...
        const resolved = yield* Provider.use.getModel(
          ProviderV2.ID.make(cloudpilotFixture.providerID),
          ModelV2.ID.make(cloudpilotFixture.modelID),
        )
        ...
        const user = {
          ...
          agent: agent.name,
          model: {
            providerID: ProviderV2.ID.make(cloudpilotFixture.providerID),
            modelID: resolved.id,
          },
      ...
      }),
    {
      config: () => {
        const fixture = loadFixture(vivgridFixture.providerID, vivgridFixture.modelID)
        return {
          enabled_providers: [cloudpilotFixture.providerID],
          provider: {
            [cloudpilotFixture.providerID]: {
