...
it.instance("plan agent denies edits except .cloudpilot/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".cloudpilot/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".cloudpilot", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.CLOUDPILOT_TEST_HOME
      process.env.CLOUDPILOT_TEST_HOME = test.directory
