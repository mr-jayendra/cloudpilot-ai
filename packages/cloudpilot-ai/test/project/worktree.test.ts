    ...
    it.instance(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(info.branch).toBe(`cloudpilot/${info.name}`)
    ...
    it.instance(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(info.branch).toBe("cloudpilot/my-feature")
        }),
      { git: true },
    )
    ...
    it.instance(
      ...
      () =>
        Effect.gen(function* () {
          ...
          const svc = yield* Worktree.Service
          yield* git(test.directory, ["branch", "cloudpilot/my-feature"])
    ...
    it.instance(
      ...
      () =>
        withCreatedWorktree(undefined, ({ info }) =>
          Effect.gen(function* () {
            expect(info.name).toBeDefined()
            expect(info.branch ?? "").toStartWith("cloudpilot/")
      ...
      () =>
        withCreatedWorktree({ name: "test-workspace" }, ({ info }) =>
          Effect.gen(function* () {
            ...
            expect(info.branch).toBe("cloudpilot/test-workspace")
          }),
        ),
      { git: true },
    )
