...
import type { QuestionRequest } from "@cloudpilot-ai/sdk/v2"
import { CloudpilotKeymapProvider, registerCloudpilotKeymap } from "@cloudpilot-ai/tui/keymap"
import {
...
async function renderFooter(
  ...
  function Harness() {
    ...
    offKeymap = registerCloudpilotKeymap(keymap, renderer, config)
...
test("direct footer shows editable prompts and additional queued work while running", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    offKeymap = registerCloudpilotKeymap(keymap, renderer, tuiConfig)
...
test("direct footer separates a lone context hint from model and command hint", async () => {
  const app = await renderFooter({
    providers: [provider()],
    currentModel: { providerID: "cloudpilot", modelID: "gpt-5" },
...
test.skip("direct custom answer submits through keymap return binding", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    off = registerCloudpilotKeymap(keymap, renderer, tuiConfig)
...
test("direct model panel renders current model selector", async () => {
  ...
  const [current] = createSignal<RunInput["model"]>({ providerID: "cloudpilot", modelID: "gpt-5" })
  ...
  try {
    ...
    expect(frame).toContain("Search")
    expect(frame).toContain("cloudpilot")
    expect(frame).toContain("GPT-5")
