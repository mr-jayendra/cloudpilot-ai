// Subprocess integration tests for `cloudpilot run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `cloudpilot.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `CLOUDPILOT_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("cloudpilot run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, cloudpilot }) =>
      Effect.gen(function* () {
        ...
        const result = yield* cloudpilot.run("say hi")
        cloudpilot.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, cloudpilot }) =>
      Effect.gen(function* () {
        ...
        const result = yield* cloudpilot.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, cloudpilot }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* cloudpilot.run("think", { extraArgs: ["--thinking"] })
        cloudpilot.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* cloudpilot.run("think again")
        cloudpilot.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, cloudpilot }) =>
      Effect.gen(function* () {
        ...
        const result = yield* cloudpilot.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, cloudpilot }) =>
      Effect.gen(function* () {
        ...
        const result = yield* cloudpilot.run("say hi", { format: "json" })
        cloudpilot.expectExit(result, 0)

        const events = cloudpilot.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ cloudpilot }) =>
      Effect.gen(function* () {
        const result = yield* cloudpilot.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, cloudpilot }) =>
      Effect.gen(function* () {
        ...
        const result = yield* cloudpilot.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, cloudpilot }) =>
      Effect.gen(function* () {
        ...
        yield* llm.text("recovered")
        const result = yield* cloudpilot.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, cloudpilot }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* cloudpilot.run("request permission", { permission: { bash: "ask" } })
        cloudpilot.expectExit(denied, 0)
        ...
        const allowed = yield* cloudpilot.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        cloudpilot.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* cloudpilot.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        cloudpilot.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, cloudpilot }) =>
      Effect.gen(function* () {
        ...
        const server = yield* cloudpilot.serve()

        const result = yield* cloudpilot.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, cloudpilot }) =>
      Effect.gen(function* () {
        const result = yield* cloudpilot.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, cloudpilot }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* cloudpilot.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
