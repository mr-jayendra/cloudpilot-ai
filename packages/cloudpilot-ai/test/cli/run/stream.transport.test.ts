...
import { CloudpilotClient, type GlobalEvent } from "@cloudpilot-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<CloudpilotClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<CloudpilotClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<CloudpilotClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<CloudpilotClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<CloudpilotClient["session"]["status"]>>["data"]>
...
function sdk(
  input: {
    ...
    subscribe?: CloudpilotClient["event"]["subscribe"]
    globalEvent?: CloudpilotClient["global"]["event"]
    promptAsync?: CloudpilotClient["session"]["promptAsync"]
    status?: CloudpilotClient["session"]["status"]
    messages?: CloudpilotClient["session"]["messages"]
    children?: CloudpilotClient["session"]["children"]
    permissions?: CloudpilotClient["permission"]["list"]
    questions?: CloudpilotClient["question"]["list"]
  } = {},
) {
  const client = new CloudpilotClient()

  const subscribe: CloudpilotClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: CloudpilotClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: CloudpilotClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: CloudpilotClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: CloudpilotClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: CloudpilotClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: CloudpilotClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: CloudpilotClient["question"]["list"] = input.questions ?? (() => ok([]))
