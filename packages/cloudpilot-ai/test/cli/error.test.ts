...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/cloudpilot.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/cloudpilot.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/cloudpilot.jsonc", dir: ".cloudpilot", suggestion: "cloudpilot" },
        expected:
          'Directory ".cloudpilot" in /tmp/cloudpilot.jsonc is not valid. Rename the directory to "cloudpilot" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/cloudpilot.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/cloudpilot.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `cloudpilot models` to list available models",
      "Or check your config (cloudpilot.json) provider/model names",
    ].join("\n")
