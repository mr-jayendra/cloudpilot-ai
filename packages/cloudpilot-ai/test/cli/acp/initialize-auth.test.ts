...
describe("cloudpilot acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ cloudpilot }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ cloudpilot }))
        ...
        expect(initialized.agentInfo?.name).toBe("CloudPilot")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ cloudpilot }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ cloudpilot })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("cloudpilot-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "cloudpilot-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.CLOUDPILOT_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ cloudpilot }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("cloudpilot-login")
