...
export function createAcpClient(input: Pick<CliFixture, "cloudpilot">, env?: Record<string, string>) {
  return Effect.gen(function* () {
    return createJsonRpcAcpClient(yield* input.cloudpilot.acp(env ? { env } : undefined))
  })
}
  ...
  return Effect.gen(function* () {
    return expectOk(
      yield* acp.request<InitializeResponse>("initialize", {
        ...
        clientInfo: { name: "cloudpilot-local-acp", version: "0.1.0" },
      }),
    )
  })
}
