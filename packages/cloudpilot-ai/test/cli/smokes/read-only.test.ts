...
describe("cloudpilot read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // CLOUDPILOT_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ cloudpilot }) =>
      Effect.gen(function* () {
        const r = yield* cloudpilot.spawn(["mcp", "list"])
        cloudpilot.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ cloudpilot }) =>
      Effect.gen(function* () {
        const r = yield* cloudpilot.spawn(["providers", "list"])
        cloudpilot.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ cloudpilot }) =>
      Effect.gen(function* () {
        const r = yield* cloudpilot.spawn(["models"])
        cloudpilot.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ cloudpilot }) =>
      Effect.gen(function* () {
        const r = yield* cloudpilot.spawn(["agent", "list"])
        cloudpilot.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh CLOUDPILOT_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ cloudpilot }) =>
      Effect.gen(function* () {
        const r = yield* cloudpilot.spawn(["session", "list"])
        cloudpilot.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ cloudpilot }) =>
      Effect.gen(function* () {
        const r = yield* cloudpilot.spawn(["stats"])
        cloudpilot.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under CLOUDPILOT_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ cloudpilot }) =>
      Effect.gen(function* () {
        const r = yield* cloudpilot.spawn(["db", "path"])
        cloudpilot.expectExit(r, 0, "db path")
