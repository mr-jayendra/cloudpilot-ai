...
// terminal sizes. The default cloudpilot tui command is excluded —
// `cloudpilot --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `cloudpilot --help` prints today;
...
// top-level help on `--help` and exits 1; not a real cloudpilot command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("cloudpilot CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ cloudpilot }) =>
      Effect.gen(function* () {
        const topLevel = yield* cloudpilot.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* cloudpilot.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`cloudpilot ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`cloudpilot ${argv.join(" ")} --help`)
        }
