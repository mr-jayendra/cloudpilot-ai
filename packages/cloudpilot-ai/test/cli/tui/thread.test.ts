...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ cloudpilot }) =>
    Effect.gen(function* () {
      const result = yield* cloudpilot.spawn(["--replay-limit", "10"])

      cloudpilot.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ cloudpilot }) =>
    Effect.gen(function* () {
      const result = yield* cloudpilot.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ cloudpilot }) =>
    Effect.gen(function* () {
      const result = yield* cloudpilot.spawn(["--mini", "--port", "4096"])
