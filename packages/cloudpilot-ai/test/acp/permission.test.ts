...
import type { Event, CloudpilotClient } from "@cloudpilot-ai/sdk/v2"
...
type PermissionReplyParams = Parameters<CloudpilotClient["permission"]["reply"]>[0]
...
function createHarness(
  ...
  } as unknown as CloudpilotClient
  const connection = {
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "cloudpilot-acp-permission-"))
