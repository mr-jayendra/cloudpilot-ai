// Subprocess test harness for the cloudpilot CLI. Spawns the real binary against
...
//
// Configuration flows through cloudpilot's built-in test affordances:
//   - CLOUDPILOT_CONFIG_CONTENT      : provider config inline, no files to find
//   - CLOUDPILOT_TEST_HOME           : pins os.homedir() → tmpdir
//   - CLOUDPILOT_DISABLE_PROJECT_CONFIG : skip walking up for cloudpilot.json
//   - CLOUDPILOT_PURE                : skip external plugin discovery + install
//   - CLOUDPILOT_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `cloudpilot.run` is fully wired. The shape supports adding more
// builders (`cloudpilot.serve(opts)`, `cloudpilot.acp(opts)`, `cloudpilot.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of CloudpilotCli.
...
import { AppProcess } from "@cloudpilot-ai/core/process"
...
const cloudpilotRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(cloudpilotRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    CLOUDPILOT_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    CLOUDPILOT_CONFIG_CONTENT: configJson,
    CLOUDPILOT_DISABLE_PROJECT_CONFIG: "1",
    CLOUDPILOT_PURE: "1",
    CLOUDPILOT_DISABLE_AUTOUPDATE: "1",
    CLOUDPILOT_DISABLE_AUTOCOMPACT: "1",
    CLOUDPILOT_DISABLE_MODELS_FETCH: "1",
    CLOUDPILOT_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `cloudpilot run`. New flags should
...
// `cloudpilot serve` is a long-lived process — it never exits on its own.
...
// `cloudpilot acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type CloudpilotCli = {
  ...
  // Spawn `cloudpilot serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `cloudpilot acp` and return a duplex JSON-RPC handle. Long-lived:
...
export type CliFixture = {
  ...
  readonly cloudpilot: CloudpilotCli
}
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("cloudpilot.spawn")(function* (args: string[], opts?: SpawnOpts) {
      ...
      const command = ChildProcess.make(process.execPath, ["run", cliEntry, ...args], {
        cwd: home,
        env: { ...env, ...opts?.env },
        extendEnv: true,
        stdin: "ignore",
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          CLOUDPILOT_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("cloudpilot.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("cloudpilot.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "cloudpilot server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `cloudpilot serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("cloudpilot.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const cloudpilot: CloudpilotCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, cloudpilot })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `cloudpilot.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "cloudpilot") {
...
// every call site. Use this for any test that needs the cloudpilot CLI fixture.
//
...
// resources (e.g. `cloudpilot.serve`) without an extra `Effect.scoped` wrapper —
