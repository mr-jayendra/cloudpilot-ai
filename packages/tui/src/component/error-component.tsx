...
export function ErrorComponent(props: { error: Error; reset: () => void; mode?: "dark" | "light" }) {
  ...
  // Safe fallback palette per mode (mirrors theme/assets/cloudpilot.json) since the
  ...
  const report = `cloudpilot ${InstallationVersion} crashed with an unexpected error.\n\nError: ${message}\n\nStack trace:\n${stack}`
              ...
              return (
                <box flexDirection="column" alignItems="center" flexShrink={0}>
                  <box
                    ...
                    backgroundColor={isCopied() ? colors.success : isSelected() ? colors.primary : undefined}
                    minWidth={15}
                    alignItems="center"
                    paddingLeft={2}
                    paddingRight={2}
        ...
        <Show when={showFooter()}>
          <box flexDirection="column" alignItems="center" flexShrink={0}>
            <text fg={colors.muted}>
              {copied() ? "Report copied — share it with the developers." : "Copy the report to share it with the developers."}
            </text>
            <text fg={colors.muted}>cloudpilot {InstallationVersion}</text>
          </box>
        </Show>
      </box>
    </box>
