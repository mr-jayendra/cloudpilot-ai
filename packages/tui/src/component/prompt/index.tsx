...
import { registerCloudpilotSpinner } from "../register-spinner"
...
import type { AssistantMessage, FilePart, UserMessage } from "@cloudpilot-ai/sdk/v2"
...
import { CLOUDPILOT_BASE_MODE, useBindings, useCommandShortcut, useLeaderActive, useCloudpilotKeymap } from "../../keymap"
...
// Rounded border glyphs matching borderStyle="rounded", but with T-junctions
// on the top corners so an open-bottomed popup above reads as one continuous
// box with the input instead of a separate floating popup.
const attachedInputBorderChars = {
  topLeft: "├",
  topRight: "┤",
  bottomLeft: "╰",
  bottomRight: "╯",
...
export function Prompt(props: PromptProps) {
  ...
  const [plusHovered, setPlusHovered] = createSignal(false)
  const slashMenuOpen = () => auto()?.visible === "/"
  const plusColor = () => {
    if (slashMenuOpen()) return theme.primary
    if (plusHovered()) return theme.textMuted
    return theme.text
  }
  ...
  const promptCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        name: "workspace.set",
        category: "Session",
        enabled: Flag.CLOUDPILOT_EXPERIMENTAL_WORKSPACES,
        slashName: "warp",
        run: () => {
          workspace.open()
        },
  ...
  // Toggles the existing slash-command menu without typing "/". Opens only on
  // empty input because selecting a slash command replaces the whole line,
  // which would destroy text the user already typed. Closing reuses the
  // existing hide path, so input text is preserved exactly as with ESC.
  ...
  const composerEmpty = createMemo(() => (store.prompt.input?.length ?? 0) === 0)
      ...
      <box ref={(r: BoxRenderable) => (anchor = r)} visible={props.visible !== false} width="100%">
        <box width="100%">
          <box
            paddingLeft={2}
            paddingRight={2}
            paddingTop={composerEmpty() ? 0 : 1}
            paddingBottom={composerEmpty() ? 0 : 1}
            flexShrink={0}
            border
            borderStyle="rounded"
            borderColor={RGBA.fromHex("#808080")}
            customBorderChars={slashMenuOpen() ? attachedInputBorderChars : undefined}
            flexGrow={1}
            width="100%"
          >
            <textarea
            ...
            >
              <box flexDirection="row" gap={1} alignItems="center">
                <box
                  flexShrink={0}
                  backgroundColor={plusHovered() ? theme.borderSubtle : undefined}
                  onMouseOver={() => {
                    if (!props.disabled) setPlusHovered(true)
                  }}
                  onMouseOut={() => setPlusHovered(false)}
                  onMouseUp={toggleSlashMenu}
                >
                  <text fg={plusColor()} attributes={TextAttributes.BOLD} selectable={false}>＋</text>
                </box>
