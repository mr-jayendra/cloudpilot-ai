...
import { useBindings, useCommandSlashes, useCloudpilotModeStack } from "../../keymap"
import { displayCharAt, mentionTriggerIndex } from "../../prompt/display"
import { formatSlashDisplay } from "../../prompt/slash-icons"
import type { FileSystemEntry } from "@cloudpilot-ai/sdk/v2"
...
export function Autocomplete(props: {
  ...
  const modeStack = useCloudpilotModeStack()
  ...
  const [files] = createResource(
    ...
    async (input) => {
      ...
      if (!result.error && result.data) {
        const width = props.anchor().width - 6
        options.push(
  ...
  const commands = createMemo((): AutocompleteOption[] => {
    ...
    for (const serverCommand of sync.data.command) {
      ...
      results.push({
        display: formatSlashDisplay(serverCommand.name + label, serverCommand.source),
        value: "/" + serverCommand.name + label,
    ...
    // Sort by the real "/<name>" syntax so icon glyphs never affect ordering.
    results.sort((a, b) => (a.value ?? a.display).localeCompare(b.value ?? b.display))

    // Pad the name column with width-aware math so a double-width glyph in a
    // terminal font can never shift the description column.
    if (results.length === 0) return results
    const max = Math.max(...results.map((item) => Bun.stringWidth(item.display)))
    return results.map((item) => ({
      ...item,
      display: item.display.padEnd(item.display.length + max + 2 - Bun.stringWidth(item.display)),
    }))
  })
        ...
        >
          {(option, index) => (
            <box
              paddingLeft={1}
              paddingRight={1}
              backgroundColor={index === store.selected ? theme.backgroundSelected : undefined}
              flexDirection="row"
              onMouseMove={() => {
