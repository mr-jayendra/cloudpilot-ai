import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@cloudpilot-ai/plugin/tui"
...
import type { useCloudpilotKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useCloudpilotKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getCloudpilotModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getCloudpilotModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
