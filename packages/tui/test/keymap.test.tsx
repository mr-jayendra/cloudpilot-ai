...
import { getCloudpilotModeStack, CLOUDPILOT_BASE_MODE, CloudpilotKeymapProvider, registerCloudpilotKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerCloudpilotKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when cloudpilot mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getCloudpilotModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getCloudpilotModeStack(keymap).push("autocomplete")
