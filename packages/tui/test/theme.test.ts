...
test("addTheme writes into module theme store", () => {
  ...
  expect(addTheme(name, DEFAULT_THEMES.cloudpilot)).toBe(true)
  expect(allThemes()[name]).toBeDefined()
})

test("addTheme keeps first theme for duplicate names", () => {
  const name = `plugin-theme-keep-${Date.now()}`
  const one = structuredClone(DEFAULT_THEMES.cloudpilot)
  const two = structuredClone(DEFAULT_THEMES.cloudpilot)
...
test("resolveTheme rejects circular color refs", () => {
  const item = structuredClone(DEFAULT_THEMES.cloudpilot)
  item.defs = { ...item.defs, one: "two", two: "one" }
  item.theme.primary = "one"
  expect(() => resolveTheme(item, "dark")).toThrow("Circular color reference")
})

test("cloudpilot dark selection uses the interaction palette", () => {
  const theme = resolveTheme(DEFAULT_THEMES.cloudpilot, "dark")
  expect(theme.backgroundSelected).toEqual(RGBA.fromHex("#424242"))
  expect(theme.selectedListItemText).toEqual(RGBA.fromHex("#ECECEC"))
})

test("cloudpilot light selection is unchanged", () => {
  const theme = resolveTheme(DEFAULT_THEMES.cloudpilot, "light")
  expect(theme.backgroundSelected).toEqual(theme.primary)
  expect(theme.selectedListItemText).toEqual(theme.background)
})

test("backgroundSelected falls back to primary when undefined", () => {
  const item = structuredClone(DEFAULT_THEMES.cloudpilot)
  delete item.theme.backgroundSelected
