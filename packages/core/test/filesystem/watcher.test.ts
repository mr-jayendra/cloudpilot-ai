...
import { Watcher } from "@cloudpilot-ai/core/filesystem/watcher"
...
const flagsLayer = ConfigProvider.layer(
  ConfigProvider.fromUnknown({
    CLOUDPILOT_EXPERIMENTAL_FILEWATCHER: "true",
    CLOUDPILOT_EXPERIMENTAL_DISABLE_FILEWATCHER: "false",
  }),
)
...
function withTmp<A, E, R>(
  ...
  return Effect.acquireRelease(
    Effect.promise(async () => {
      ...
      await $`git config user.email test@cloudpilot.test`.cwd(tmp.path).quiet()
