...
import { define } from "@cloudpilot-ai/plugin/v2/effect"
...
import { LocationServiceMap } from "@cloudpilot-ai/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "cloudpilot.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "cloudpilot.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
