...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@cloudpilot/acme")).toBe("@cloudpilot/acme")
    expect(Npm.sanitize("@cloudpilot/acme@1.0.0")).toBe("@cloudpilot/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/opencode/acme.git"
    const expected = win ? "acme@git+https_//github.com/cloudpilot/acme.git" : spec
