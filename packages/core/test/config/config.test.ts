...
import { ConfigProvider } from "@cloudpilot-ai/core/config/provider"
...
import { ConfigMigrateV1 } from "@cloudpilot-ai/core/v1/config/migrate"
...
import { Policy } from "@cloudpilot-ai/core/policy"
...
describe("Config", () => {
  ...
  it.live("loads cloudpilot JSON and JSONC files from lowest to highest priority", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "cloudpilot.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "cloudpilot.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "cloudpilot.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "cloudpilot.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "cloudpilot.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "cloudpilot.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
      ...
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.writeFile(
              path.join(global, "cloudpilot.json"),
              JSON.stringify({
                experimental: { policies: [{ effect: "deny", action: "provider.use", resource: "openai" }] },
              }),
            )
  ...
  it.live("loads global, ancestor, and .cloudpilot configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".cloudpilot"), { recursive: true })
            await fs.mkdir(path.join(directory, ".cloudpilot"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "cloudpilot.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "cloudpilot.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "cloudpilot.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "cloudpilot.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "cloudpilot.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".cloudpilot", "cloudpilot.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".cloudpilot", "cloudpilot.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".cloudpilot")),
              AbsolutePath.make(path.join(directory, ".cloudpilot")),
            ])
