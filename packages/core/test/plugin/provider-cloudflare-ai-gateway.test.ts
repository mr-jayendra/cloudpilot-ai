...
import { CloudflareAIGatewayPlugin } from "@cloudpilot-ai/core/plugin/provider/cloudflare-ai-gateway"
      ...
      Effect.gen(function* () {
        ...
        yield* aisdk.runSDK({
          ...
          options: {
            ...
            metadata: { invoked_by: "test", project: "cloudpilot" },
            cacheTtl: 300,
            cacheKey: "cache-key",
            skipCache: true,
            collectLog: false,
        ...
        expect(aiGatewayCalls[0]).toEqual({
          ...
          options: {
            ...
            collectLog: false,
            headers: {
              "User-Agent": expect.stringContaining("cloudpilot/"),
            },
          },
        })
      }),
      ...
      Effect.gen(function* () {
        ...
        yield* aisdk.runSDK({
          ...
          options: {
            name: "cloudflare-ai-gateway",
            headers: {
              "cf-aig-metadata": JSON.stringify({ invoked_by: "header", project: "cloudpilot" }),
            },
          },
        })

        expect(aiGatewayCalls[0]?.options).toMatchObject({
          metadata: { invoked_by: "header", project: "cloudpilot" },
        })
      }),
    ),
  )
