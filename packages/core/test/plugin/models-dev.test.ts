...
import { ModelsDevPlugin } from "@cloudpilot-ai/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.CLOUDPILOT_MODELS_PATH,
          disabled: Flag.CLOUDPILOT_DISABLE_MODELS_FETCH,
        }
        Flag.CLOUDPILOT_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
        Flag.CLOUDPILOT_DISABLE_MODELS_FETCH = true
        return previous
      }),
      () =>
        Effect.gen(function* () {
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.CLOUDPILOT_MODELS_PATH = previous.path
          Flag.CLOUDPILOT_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
