...
import { DynamicProviderPlugin } from "@cloudpilot-ai/core/plugin/provider/dynamic"
...
function tempEntrypoint(source: string) {
  return Effect.acquireRelease(
    Effect.promise(async () => {
      const directory = await fs.mkdtemp(path.join(os.tmpdir(), "cloudpilot-provider-dynamic-"))
