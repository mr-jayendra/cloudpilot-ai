...
import { PluginHost } from "@cloudpilot-ai/core/plugin/host"
import { CloudpilotPlugin } from "@cloudpilot-ai/core/plugin/provider/cloudpilot"
...
const addPlugin = Effect.fn(function* (http?: HttpClient.HttpClient) {
  ...
  yield* CloudpilotPlugin.effect(host).pipe(
...
describe("CloudpilotPlugin", () => {
  it.effect("registers account and service account methods", () =>
    Effect.gen(function* () {
      yield* addPlugin()
      expect((yield* (yield* Integration.Service).get(Integration.ID.make("cloudpilot")))?.methods).toEqual([
        {
          id: Integration.MethodID.make("device"),
          type: "oauth",
          label: "CloudPilot Console account",
        },
  ...
  it.effect("resolves origin-rooted device verification URLs", () =>
    Effect.gen(function* () {
      ...
      const attempt = yield* integration.connection.oauth({
        integrationID: Integration.ID.make("cloudpilot"),
  ...
  it.live("loads providers and models from the connected CloudPilot server", () =>
        ...
        Effect.gen(function* () {
          ...
          const provider = required(
            yield* eventually(
              ...
              (item) => item?.integrationID === Integration.ID.make("cloudpilot"),
            ),
          )
          expect(provider).toMatchObject({
            name: "Remote",
            integrationID: "cloudpilot",
            api: {
              type: "aisdk",
  ...
  it.effect("uses a public key and disables paid models without credentials", () =>
    withEnv({ CLOUDPILOT_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
            ...ProviderV2.Info.empty(ProviderV2.ID.cloudpilot),
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.cloudpilot)).request.body.apiKey).toBe("public")
        expect(required(yield* catalog.model.get(ProviderV2.ID.cloudpilot, ModelV2.ID.make("paid"))).enabled).toBe(false)
      }),
    ),
  )
  ...
  it.effect("keeps free models without credentials", () =>
    withEnv({ CLOUDPILOT_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.cloudpilot)).request.body.apiKey).toBe("public")
        expect(required(yield* catalog.model.get(ProviderV2.ID.cloudpilot, ModelV2.ID.make("free"))).enabled).toBe(true)
      }),
    ),
  )
  ...
  it.effect("treats output-only cost as free without credentials", () =>
    withEnv({ CLOUDPILOT_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.cloudpilot)).request.body.apiKey).toBe("public")
        expect(required(yield* catalog.model.get(ProviderV2.ID.cloudpilot, ModelV2.ID.make("output-only"))).enabled).toBe(
          true,
        )
      }),
    ),
  ...
  it.effect("uses CLOUDPILOT_API_KEY as credentials", () =>
    withEnv({ CLOUDPILOT_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.cloudpilot)).request.body.apiKey).toBeUndefined()
        expect(required(yield* catalog.model.get(ProviderV2.ID.cloudpilot, ModelV2.ID.make("paid"))).enabled).toBe(true)
      }),
    ),
  )

  it.effect("uses configured provider env vars as credentials", () =>
    withEnv({ CLOUDPILOT_API_KEY: undefined, CUSTOM_CLOUDPILOT_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        ...
        yield* integrations.transform((editor) => {
          editor.method.update({
            integrationID: Integration.ID.make("cloudpilot"),
            method: { type: "env", names: ["CUSTOM_CLOUDPILOT_API_KEY"] },
          })
        })
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
  ...
  it.effect("uses configured apiKey as credentials", () =>
    withEnv({ CLOUDPILOT_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.cloudpilot)).request.body.apiKey).toBe("configured")
        expect(required(yield* catalog.model.get(ProviderV2.ID.cloudpilot, ModelV2.ID.make("paid"))).enabled).toBe(true)
      }),
    ),
  )

  it.effect("ignores non-cloudpilot providers and models", () =>
    withEnv({ CLOUDPILOT_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
  ...
  it.effect("prefers gpt-5-nano as the cloudpilot small model", () =>
    Effect.gen(function* () {
      const catalog = yield* Catalog.Service
      const providerID = ProviderV2.ID.cloudpilot
