...
import { GoogleVertexPlugin } from "@cloudpilot-ai/core/plugin/provider/google-vertex"
  ...
  it.effect("ignores OpenAI-compatible providers that are not Google Vertex", () =>
    Effect.gen(function* () {
      ...
      yield* catalog.transform((catalog) =>
        catalog.provider.update(ProviderV2.ID.cloudpilot, (provider) => {
          provider.api = {
            type: "aisdk",
      ...
      const provider = required(yield* catalog.provider.get(ProviderV2.ID.cloudpilot))
