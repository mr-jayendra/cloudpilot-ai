...
export function fileLogger(file = path.join(Global.Path.log, "cloudpilot.log"), id: string = runID) {
...
export function minimumLogLevel() {
  const value = process.env.CLOUDPILOT_LOG_LEVEL?.toUpperCase()
  const levels = {
    DEBUG: "Debug",
    INFO: "Info",
    WARN: "Warn",
...
export function loggers() {
  return process.env.CLOUDPILOT_PRINT_LOGS === "1" ? [fileLogger(), stderrLogger] : [fileLogger()]
}
