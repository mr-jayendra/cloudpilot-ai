...
import type { IntegrationOAuthMethodRegistration } from "@cloudpilot-ai/plugin/v2/effect/integration"
import { define } from "@cloudpilot-ai/plugin/v2/effect/plugin"
import type { CredentialValue } from "@cloudpilot-ai/sdk/v2/types"
...
export const CloudpilotPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "cloudpilot",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("CloudpilotPlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("cloudpilot")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load CloudPilot provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("cloudpilot", (integration) => {
        integration.name = "CloudPilot"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "cloudpilot", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("cloudpilot")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("cloudpilot")
      ...
      const item = catalog.provider.get(ProviderV2.ID.cloudpilot)
      if (!item) return
      const hasKey = Boolean(process.env.CLOUDPILOT_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("cloudpilot")),
