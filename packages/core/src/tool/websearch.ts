...
import { ToolFailure } from "@cloudpilot-ai/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@cloudpilot/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.CLOUDPILOT_WEBSEARCH_PROVIDER === "exa" || process.env.CLOUDPILOT_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.CLOUDPILOT_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("CLOUDPILOT_EXPERIMENTAL") || truthy("CLOUDPILOT_ENABLE_EXA") || truthy("CLOUDPILOT_EXPERIMENTAL_EXA"),
    enableParallel: truthy("CLOUDPILOT_ENABLE_PARALLEL") || truthy("CLOUDPILOT_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `cloudpilot/${InstallationVersion}`,
