...
export interface Interface {
   ...
   *
   * This exists while the old cloudpilot project service and this core project
...
export class Service extends Context.Service<Service, Interface>()("@cloudpilot/ProjectV2") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const cached = Effect.fnUntraced(function* (dir: string) {
      return yield* fs.readFileString(path.join(dir, "cloudpilot")).pipe(
    ...
    const commit = Effect.fn("Project.commit")(function* (input: { store: AbsolutePath; id: ID }) {
      yield* fs.writeFileString(path.join(input.store, "cloudpilot"), input.id).pipe(Effect.ignore)
    })
